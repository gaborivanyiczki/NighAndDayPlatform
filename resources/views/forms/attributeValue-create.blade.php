<form action="{{isset($route)?$route:route('dashboard.attributes.values.store')}}" method="POST">
    {{csrf_field()}}
    <input type="hidden" name="_method" value="{{isset($method)?$method:'POST'}}"/>

    <div class="form-group">
        <label for="Value" class="font-weight-bold">Valoare predefinita</label>
        <input type="text" class="form-control {{ $errors->has('Value') ? ' is-invalid' : '' }}" name="Value" id="Value" value="" placeholder="" maxlength="255" required="required" >
        @if($errors->has('Value'))
